...
export default function TypingInterface({
  ...
  useEffect(() => {
    if (codeDisplayRef.current) {
      const currentCharSpan = codeDisplayRef.current.querySelector(
  ...
  const renderCode = () => {
    return code.split("").map((char, index) => {
      ...
      if (index < userInput.length) {
        ...
        className = "text-white bg-neon-cyan/30 animate-pulse"; // Current
      } else {
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex space-x-6">
              <span className="text-gray-400">
                ...
                <span className="text-neon-cyan font-semibold">
                  {Math.round((userInput.length / code.length) * 100)}%
                </span>
              </span>
              <span className="text-gray-400">
                Errors:{" "}
      ...
      <div className="flex justify-center space-x-4">
        <Button
          onClick={() => {
            setTimeLeft(60); // ⏱ Reset timer manually
            onReset();
          }}
          variant="outline"
        ...
        <Card className="bg-blue-500/10 border-neon-blue">
          <CardContent className="p-4 text-center">
            <p className="text-sm text-blue-300">
              💡 <strong>Tip:</strong> The code display will scroll automatically
              as you type. Focus on the typing area below and match the code
              exactly!
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-green-500/10 border-green-500">
          <CardContent className="p-4 text-center">
            <p className="text-sm text-green-300">
              🎉 <strong>Great job!</strong> You've completed this challenge.
              Check your results and try the next one!
            </p>
          </CardContent>
        </Card>
      )}
